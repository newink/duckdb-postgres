...
if [ "$LINUX_CI_IN_DOCKER" = "1" ]; then
    ...
    if command -v yum >/dev/null 2>&1; then
        ...
        # Install libpq runtime library (not -devel) for dynamic loading
        yum install -y postgresql-libs 2>/dev/null
        ...
        # Install libpq runtime library for dynamic loading
        apt-get install -y libpq5 2>/dev/null
    elif command -v apk >/dev/null 2>&1; then
        echo "Detected apk package manager (Alpine)"
        apk add --no-cache krb5-dev krb5-libs krb5 krb5-static 2>/dev/null
        # Install libpq runtime library for dynamic loading
        apk add --no-cache postgresql-libs 2>/dev/null
    else
    ...
    echo "Checking for runtime libpq libraries..."
    ls -la /usr/lib64/libpq.so* /usr/lib/libpq.so* /usr/local/lib/libpq* 2>/dev/null || echo "No libpq runtime libraries found"
